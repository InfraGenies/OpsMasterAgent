...
**Two-pass split (revised again):** a real Bedrock call asked to produce both priced tiers *and* the full
`architecture_recommendation` in one response proved unreliable — confirmed live twice (a timeout and a
hard failure where the model returned only one tier even after the validation retry). The full recommendation
plus two complete priced options was simply too large/complex a single ask. This skill is now used in **two
separate LLM calls** per enterprise_mode request, each a smaller, more focused task than the single call
this replaced:
- **Pass 1** produces the full-posture `"high_availability"` option plus the complete
  `architecture_recommendation` — this is the same size/shape of call the original single-tier design
  already handled reliably.
- **Pass 2** is given Pass 1's output as ground truth and produces *only* a cost-reduced `"economy"`
  option — a narrow, constrained follow-up task, not a fresh full analysis.
- `planner.ts`'s `runEnterprisePlanner` calls this skill twice, prefixing a short "THIS IS PASS 1/2 of 2"
  directive each time; sections below are labeled which pass they apply to.

## Content (load this verbatim, append to the planner's user-turn note when `enterprise_mode` — the
   calling code prefixes a pass-specific directive before this block on each of the two calls)
...
input combination, not just canned examples. This is the FULL, uncapped set — Pass 1's
architecture_recommendation always reflects it regardless of what Pass 2 ends up dropping for cost.

[PASS 1 ONLY] REASONING DEPTH (this is the part a fixed lookup table can't do — do this genuinely, not by
rote): for every control you recommend in the dr_ha, data_protection, or network category (Aurora Global
...
SAME archetype-governance and compliance-overlay controls as Pass 1 (these are never optional-for-cost —
PCI-DSS/HIPAA requirements and org-scale governance controls don't relax for a cheaper tier). Drop ONLY the
