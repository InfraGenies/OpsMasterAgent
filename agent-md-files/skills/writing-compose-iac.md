...
**Used by:** `03-iac-generator.md` (IaC Generator), always — needed whether the request ends up matching a
catalog template or falling through to the freeform path, since it can't know in advance which will happen.

Not extracted from an existing prompt — this documents the conventions the vetted templates in
`templates/catalog.ts`/`composeBuilder.ts` already encode (healthchecks, the nginx-sidecar rule, secrets),
so freeform output follows the same conventions instead of improvising blind. New file.
