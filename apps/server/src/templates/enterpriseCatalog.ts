...
#
# Cost safety: after "terraform apply" succeeds, run .\\schedule-auto-destroy.ps1 (defaults to
# 45 minutes, pass -Minutes to change it) to auto-run "terraform destroy" later so a forgotten
# test deployment doesn't keep billing. Run .\\cancel-auto-destroy.ps1 to cancel it and keep
# resources running longer instead.
...
`;
  ...
  const taskName = `OpsMasterAgent-AutoDestroy-${input.environmentName}`;

  const scheduleAutoDestroyPs1 = `<#
.SYNOPSIS
  Cost-safety timer: schedules "terraform destroy -auto-approve" to run automatically in this
  same folder after a delay, so a real AWS deploy started for a quick test doesn't keep billing
  if you forget to tear it down yourself.

.DESCRIPTION
  Run this AFTER "terraform apply" has succeeded. It registers a ONE-SHOT Windows Task Scheduler
  job (not a foreground sleep loop) so it still fires even if this terminal window is closed,
  but it does NOT fire if this machine is asleep/off/logged-out at the scheduled time. For a test
  you don't expect to outlive your own working session, that's normally fine; if you might close
  your laptop, destroy manually before doing so instead of relying on this.

.PARAMETER Minutes
  Delay before auto-destroy runs. Defaults to 45.
